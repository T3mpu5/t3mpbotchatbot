    ...
    public class Main
    {
        ...
        private static void AddNewStats()
        {
            lock (_lockstats)
            {
                ...
                {
                    ele.Add(new XAttribute("Archery", "5"),
                        new XAttribute("Looting", "5"));
                }
                NStats.Save(@"Data\Stats.xml");
            }
        ...
        private void ChatCommands(OnMessageReceivedArgs e, string channel, string[] args)
        {
            ...
            if (args[0] == "!LOOT" || args[0] == "!BAG")
            {
                ...
                client.SendMessage(channel, "/me : Your loot bag has: [1] - " + Slot1 + ", [2] - " + Slot2 + ", [3] - " + Slot3 + ", [4] - " + Slot4 + ", [5] - " + Slot5 + ".");
            }
            if (args[0] == "!SHARDS")
            {
                CheckNoXP(channel, e.ChatMessage.Username.ToUpper());
                ...
                client.SendMessage(channel, "/me : Your shard bag has: [Steel] - " + Steel + " Shards, [Moonstone] - " + Moonstone + " Shards, [Fire] - " + Fire + " Shards, [Enchanted] - " + Enchanted + " Shards.");
            }
            if (args[0] == "!SMELT")
            {
                CheckNoXP(channel, e.ChatMessage.Username.ToUpper());
                ...
                {
                    if (!string.IsNullOrEmpty(args[1]) && args[1].All(Char.IsDigit) && Enumerable.Range(1, 5).Contains(Convert.ToInt32(args[1])))
                    {
                        string loot = null;
                        if (args[1] == "1")
                        {
                            loot = adventure.GetLootSlot1(channel, e.ChatMessage.Username.ToUpper());
                        }
                        if (args[1] == "2")
                        {
                            loot = adventure.GetLootSlot2(channel, e.ChatMessage.Username.ToUpper());
                        }
                        if (args[1] == "3")
                        {
                            loot = adventure.GetLootSlot3(channel, e.ChatMessage.Username.ToUpper());
                        }
                        if (args[1] == "4")
                        {
                            loot = adventure.GetLootSlot4(channel, e.ChatMessage.Username.ToUpper());
                        }
                        if (args[1] == "5")
                        {
                            loot = adventure.GetLootSlot5(channel, e.ChatMessage.Username.ToUpper());
                        }
                        if (loot == null)
                        {
            ...
            if (args[0] == "!THROW")
            {
                ...
                else
                {
                    ...
                    else if (Enumerable.Range(1, 5).Contains(Convert.ToInt32(args[1])))
                    {
                        string loot = null;
                        if (args[1] == "1")
                        {
                    ...
                    {
                        client.SendMessage(channel, "/me : You only have 5 item slots, pick a slot 1 to 5.");
                    }
                }
            }
            if (args[0] == "!EQUIP")
            ...
            if (args[0] == "!STATS" || args[0] == "!SKILLS")
            {
                ...
                client.SendMessage(channel, "/me : " + e.ChatMessage.DisplayName + " (Lvl " + LookUpLvl(channel, e.ChatMessage.Username.ToUpper()) + ") has HP: (" + HP + "), Strength: (" + STR + "), Archery: (" + ARC + "), Endurance: (" + END + "), Intelligence: (" + INT + "), Dexterity: (" + DEX + "), Looting: (" + LOO + ")");
            }
            if (args[0] == "!LVLON")
            {
                CheckNoXP(channel, e.ChatMessage.Username.ToUpper());
